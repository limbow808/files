...
SDE_BZ2_PATH   = SDE_PATH + ".bz2"
SDE_STAMP_PATH = SDE_PATH + ".last-modified"
CREST_PATH     = os.path.join(os.path.dirname(__file__), "crest.db")

SDE_URL = "https://www.fuzzwork.co.uk/dump/sqlite-latest.sqlite.bz2"


def _fetch_sde(force: bool = False) -> bool:
    """
    Download sqlite-latest.sqlite.bz2 from Fuzzwork if a newer version is
    available (checked via the Last-Modified response header) and decompress it.

    Returns True if the SDE was updated, False if already current.
    """
    print("  Checking Fuzzwork SDE for updates...", end="", flush=True)

    # Read the previously saved Last-Modified stamp
    saved_stamp = ""
    if os.path.exists(SDE_STAMP_PATH):
        with open(SDE_STAMP_PATH, "r") as f:
            saved_stamp = f.read().strip()

    # HEAD request to compare Last-Modified without downloading
    if not force and saved_stamp and os.path.exists(SDE_PATH) and os.path.getsize(SDE_PATH) > 0:
        try:
            req = urllib.request.Request(SDE_URL, method="HEAD")
            with urllib.request.urlopen(req, timeout=15) as resp:
                remote_stamp = resp.headers.get("Last-Modified", "")
            if remote_stamp and remote_stamp == saved_stamp:
                print(f" already up to date ({saved_stamp})")
                return False
        except Exception as e:
            print(f" HEAD check failed ({e}), proceeding with existing file")
            return False

    # Download the bz2
    print(f" downloading from {SDE_URL}")
    try:
        def _progress(block_num, block_size, total_size):
            if total_size > 0:
                pct = min(100, block_num * block_size * 100 // total_size)
                print(f"\r  Downloading SDE... {pct}%", end="", flush=True)

        urllib.request.urlretrieve(SDE_URL, SDE_BZ2_PATH, reporthook=_progress)
        print()  # newline after progress
    except Exception as e:
        print(f"\n  ERROR: Failed to download SDE: {e}")
        sys.exit(1)

    # Decompress bz2 → sqlite
    print("  Decompressing SDE...", end="", flush=True)
    try:
        with bz2.open(SDE_BZ2_PATH, "rb") as src, open(SDE_PATH, "wb") as dst:
            shutil.copyfileobj(src, dst)
        os.remove(SDE_BZ2_PATH)
        print(f" done ({os.path.getsize(SDE_PATH) // 1_000_000} MB)")
    except Exception as e:
        print(f"\n  ERROR: Failed to decompress SDE: {e}")
        sys.exit(1)

    # Save the new Last-Modified stamp
    try:
        req = urllib.request.Request(SDE_URL, method="HEAD")
        with urllib.request.urlopen(req, timeout=15) as resp:
            remote_stamp = resp.headers.get("Last-Modified", "")
        if remote_stamp:
            with open(SDE_STAMP_PATH, "w") as f:
                f.write(remote_stamp)
    except Exception:
        pass  # non-fatal
...
def _seed_invention(sde: sqlite3.Connection, crest: sqlite3.Connection) -> int:
    ...
    # probability was moved to industryActivityProbabilities in newer SDE builds
    try:
        prod_rows = sde.execute("""
            SELECT p.typeID AS t1_bp_id, p.productTypeID AS t2_bp_id,
                   COALESCE(pr.probability, 0.34) AS probability
            FROM   industryActivityProducts p
            LEFT JOIN industryActivityProbabilities pr
                   ON pr.typeID = p.typeID
                  AND pr.activityID = p.activityID
                  AND pr.productTypeID = p.productTypeID
            WHERE  p.activityID = 8
        """).fetchall()
    except Exception as e:
