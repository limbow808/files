...
def _load_calc_cache_from_disk() -> None:
    """Load persisted calculator cache from disk; discard stale entries."""
    ...
    try:
        ...
        now   = time.time()
        fresh = {k: v for k, v in saved.items()
                 if (now - v.get("generated_at", 0)) < CALC_CACHE_TTL}
        _calc_cache.update(fresh)
        if fresh:
            _calc_ready = True
            print(f"  [cache] Restored {len(fresh)} calculator result(s) from disk.")
...
def api_top_performers():
    ...
    if not _ESI_JOBS_CACHE or (time.time() - _ESI_JOBS_CACHE_TS) >= _ESI_JOBS_TTL:
        try:
            ...
            print(f"[top-performers] Jobs cache refresh failed: {_e}")
    ...
    if not _ESI_ORDERS_CACHE or (time.time() - _ESI_ORDERS_CACHE_TS) >= _ESI_ORDERS_TTL:
        try:
            ...
            print(f"[top-performers] Orders cache refresh failed: {_e}")
    ...
    if not _calc_is_fresh(preferred_key_exact):
        try:
            ...
            print(f"[top-performers] Calculator cache refresh failed: {_e}")
    ...
    def _build_downstream(result: dict) -> dict | None:
        ...
        for material in result.get("material_breakdown", []):
            ...
            if have < needed:
                ...
                missing_mats_est_cost += unit_cost * (needed - have)
                inbound_missing_m3 += unit_volume * (needed - have)
        ...
        return {
            ...
            "material_breakdown": result.get("material_breakdown", []),
    ...
    def _build_invention_candidate(result: dict, base_item: dict) -> dict | None:
        ...
        candidate.update({
            ...
            "start_at": now_ts,
            "manufacture_at": now_ts + science_total_secs,
    ...
    if len(mfg_items_raw) < max_jobs:
        ...
        fallback_pool = [item for item in fallback_mfg_candidates if item["rec_id"] not in seen_rec_ids]
    ...
    science_pool = [item for item in science_candidates if float(item.get("profit_per_cycle", 0) or 0) > 0]
...
def api_bpo_market_scan():
    """Return ranked BPO acquisition opportunities from market plus contracts."""
    try:
        ...
        raw_matches = _cc.query_bp_contracts(wanted_bp_ids, region_id=region_id, bpo_only=True) if stats["items_fetched"] > 0 else []
        best_contract_by_bpid: dict[int, dict] = {}
        for row in raw_matches:
            blueprint_id = int(row.get("type_id") or 0)
            if blueprint_id <= 0:
                continue
            current = best_contract_by_bpid.get(blueprint_id)
            if current is None:
                copied = dict(row)
                copied["listing_count"] = 1
                best_contract_by_bpid[blueprint_id] = copied
                continue
            current["listing_count"] = int(current.get("listing_count") or 1) + 1
            if float(row.get("price") or 0) < float(current.get("price") or 0):
                copied = dict(row)
                copied["listing_count"] = current["listing_count"]
                best_contract_by_bpid[blueprint_id] = copied
        ...
        for blueprint_id in wanted_bp_ids:
            ...
            contract_row = best_contract_by_bpid.get(blueprint_id)
            if market_price > 0:
                market_match_count += 1
            if market_price <= 0 and contract_row is None:
                continue
        ...
        return jsonify({
            ...
            "contract_matches": len(best_contract_by_bpid),
            ...
            "from_cache": True,
            "wallet_total_isk": wallet_total_isk,
            "message": None if results else "No acquisition listings found for profitable BPOs in the selected market/cache.",
        })
    except Exception as e:
        import traceback
        traceback.print_exc()
...
def _build_bpo_acquisition_row(
    *,
    ...
    contract_price = float(contract_row.get("price") or 0) if contract_row else 0.0
    market_price = float(market_price or 0)
    has_market = market_price > 0
    has_contract = contract_price > 0
    ...
    listing_count = 0
    chosen_runs = -1
    if has_contract and (not has_market or contract_price <= market_price):
        source = "contract"
        acquisition_price = contract_price
        me = int(contract_row.get("material_efficiency") or 0)
        te = int(contract_row.get("time_efficiency") or 0)
        contract_id = contract_row.get("contract_id")
        listing_count = int(contract_row.get("listing_count") or 1)
        chosen_runs = int(contract_row.get("runs") or -1)
    ...
    available_sources = []
    if has_market:
        available_sources.append("market")
    if has_contract:
        available_sources.append("contract")
    ...
    return {
        ...
        "name": calc_row.get("name") or sde_row.get("name", "?"),
        "source": source,
        "source_label": "JITA MARKET" if source == "market" else "CONTRACT",
        "available_sources": available_sources,
        "market_available": has_market,
        "contract_available": has_contract,
        "market_price": round(market_price, 2) if has_market else None,
        "contract_price": round(contract_price, 2) if has_contract else None,
        ...
        "listing_count": listing_count if has_contract else 0,
        ...
        "cheapest_price": round(min([p for p in (market_price, contract_price) if p > 0]), 2) if (has_market or has_contract) else None,
...
def api_blueprints_esi():
    ...
    try:
        ...
        if not corp_esi_loaded_any and CORP_BPO_TYPE_IDS:
...
def _get_max_jobs(running_fallback: int = 0) -> int:
    ...
    global _MAX_JOBS_CACHE, _MAX_JOBS_CACHE_TS, _MFG_TIME_MODIFIER
    ...
    try:
        ...
        def _fetch_slots(cid, headers):
            try:
                ...
                slots = 1 + skill_map.get(MASS_PROD, 0) + skill_map.get(ADV_MASS_PROD, 0)
                mfg_mod = (
                    (1.0 - 0.04 * skill_map.get(INDUSTRY, 0))
                    * (1.0 - 0.03 * skill_map.get(ADV_INDUSTRY, 0))
                )
                return slots, mfg_mod
            except Exception:
                return 1, 1.0
        ...
        if auth_headers:
            with ThreadPoolExecutor(max_workers=len(auth_headers)) as pool:
                for slots, mfg_mod in pool.map(lambda x: _fetch_slots(*x), auth_headers):
                    max_jobs += slots
                    best_mfg_modifier = min(best_mfg_modifier, mfg_mod)
        else:
