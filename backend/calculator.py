...
COSTS ACCOUNTED FOR:
  ...
  3. Broker fee       - % to list on market (default 3%, reduced by Broker Relations skill)
    4. System Cost Index - job installation cost (activity-specific SCI)
...
STRUCTURE_ROLE_BONUS_BY_TYPE = {
    35825: {"manufacturing": -0.03, "copying": -0.03, "invention": -0.03},  # Raitaru
    35826: {"manufacturing": -0.04, "copying": -0.04, "invention": -0.04},  # Azbel
    35827: {"manufacturing": -0.05, "copying": -0.05, "invention": -0.05},  # Sotiyo
    35835: {"manufacturing":  0.0,  "copying":  0.0,  "invention":  0.0},   # Athanor
    35836: {"manufacturing":  0.0,  "copying":  0.0,  "invention":  0.0},   # Tatara
}
...
def _role_bonus_for_activity(activity: str, cfg: dict) -> float:
    ...
    if activity == "invention" and cfg.get("structure_role_bonus_invention") is not None:
        return float(cfg.get("structure_role_bonus_invention") or 0.0)
...
def calculate_industry_job_cost(
    ...
    if activity in ("copying", "invention"):
        multiplier_key = "copying_activity_multiplier" if activity == "copying" else "invention_activity_multiplier"
        activity_multiplier = float(cfg.get(multiplier_key, 0.02) or 0.02)
        base_cost = eiv * activity_multiplier
        rig_key = "rig_bonus_copy" if activity == "copying" else "rig_bonus_invention"
        rig_bonus = float(cfg.get(rig_key, 0.0) or 0.0)
    else:
...
def calculate_profit(blueprint: dict, prices: dict, config_override: dict = None,
    ...
    if is_t2 and invention_prices is not None:
        inv_result = calculate_invention_cost(bp_name, prices=invention_prices)
        if inv_result:
            invention_cost_per_run = float(inv_result["cost_per_run"] or 0.0)
            invention_detail = dict(inv_result)

            invention_sci = float(
                cfg.get("invention_system_cost_index")
                if cfg.get("invention_system_cost_index") is not None
                else cfg.get("system_cost_index", 0.0)
            )
            ...
            invention_cost_per_run += invention_job_cost_per_run
            invention_cost = invention_cost_per_run
            invention_detail.update({
                ...
                "total_cost_per_run": invention_cost_per_run,
            })
